...
我们打算通过国际互联网解决这个问题。MFC 是一项在全世界范围内广泛应用且历史悠久的技术，我们遇到的 MFC 问题，大概率在国际互联网上能够搜索到这个问题，并且找到相应的解决方案。果然通过搜索我们发现了 https://www.dll-files.com/ 这个网站，缺失的动态链接库可以在这个网站上搜索到并下载。
...
需要注意的是，我们将 mfc140ud.dll 拷贝到运行环境的 ``C:\Windows\System32`` 目录，这个路径取决于 Windows 操作系的安装目录。如 :numref:`fig_osc_LookFor_SystemRoot` 所示，运行 ``echo %SystemRoot%`` 命令，可以查看 Windows 操作系的安装目录。
